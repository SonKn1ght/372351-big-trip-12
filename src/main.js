...
//самое подозрительное место, сделал так что б закинуть элемент вторым потомком после скрытого заголовка в базовой разметке
const сontrolHiddenTitleElement = mainElement.querySelector(`.trip-controls .visually-hidden`);

render(mainElement, createTripInfoTemplate(), `afterbegin`);
render(сontrolHiddenTitleElement, createTabsTemplate(), `afterend`);
